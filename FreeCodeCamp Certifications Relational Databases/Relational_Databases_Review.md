...
# Insert student data
psql -U "$DB_USER" -d "$DB_NAME" -c "INSERT INTO students (name, age, major) VALUES ('Alice', 20, 'CS');"
...
psql -U "$DB_USER" -d "$DB_NAME" -c "INSERT INTO students (name, age) VALUES ('$STUDENT_NAME', $AGE);"
...
STUDENT_COUNT=$(psql -U "$DB_USER" -d "$DB_NAME" -t -A -c "SELECT COUNT(*) FROM students;")
...
RESULTS=$(psql -U "$DB_USER" -d "$DB_NAME" -t -A -F"," -c "SELECT name, age FROM students LIMIT 3;")
echo "Top 3 students:"
echo "$RESULTS"
...
psql -U "$DB_USER" -d "$DB_NAME" -t -A -F"," -c "SELECT name, major FROM students;" | while IFS="," read -r name major
do
