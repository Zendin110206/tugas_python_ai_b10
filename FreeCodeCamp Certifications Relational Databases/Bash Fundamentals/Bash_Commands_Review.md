...
* **PowerShell / Command Prompt / Windows Terminal:** Options for accessing the command line on Windows.
...
* Check the current directory with `pwd` before moving, copying, or deleting files.
* Use `ls` or `find` to verify targets before running destructive commands.
* Be especially careful with `rm -r` because it removes directories and their contents recursively.
* Prefer documenting important command sequences after completing a workshop, rather than committing raw copied instructions.
