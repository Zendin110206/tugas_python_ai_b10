...
* **AUTO_INCREMENT Attribute:** This attribute is used in MySQL to create auto-incrementing integer columns. In PostgreSQL, the comparable beginner-friendly option used in this workshop is `SERIAL`.
...
* **Primary Keys:** Give every main table a primary key so each row can be identified reliably.
* **Foreign Keys:** Use foreign keys to connect related tables and protect referential integrity.
* **Junction Tables:** Use a junction table for many-to-many relationships instead of repeating columns or duplicating lookup data.
* **Specific WHERE Clauses:** Be careful with `UPDATE` and `DELETE`. A broad `WHERE` condition can change more rows than intended.
* **Readable Units:** Include units in column names when useful, such as `height_in_cm` or `weight_in_kg`.
* **Verification:** Use `\d`, `SELECT`, and row counts to verify schema and data after important changes.

---

## Key Takeaways

* Relational databases organize structured data into connected tables.
* Primary keys identify rows, while foreign keys connect rows across tables.
* Constraints such as `NOT NULL`, `UNIQUE`, and `PRIMARY KEY` improve data integrity.
* PostgreSQL `SERIAL` is useful for simple auto-incrementing IDs in beginner projects.
* Relationship type determines table design:
  * one-to-one can use a foreign key with `UNIQUE`
  * one-to-many keeps the foreign key on the many side
  * many-to-many should use a junction table
* Joins are used to read connected data across multiple tables.
