...
echo "$($PSQL "SELECT course FROM students FULL JOIN majors ON students.major_id = majors.major_id FULL JOIN majors_courses ON majors.major_id = majors_courses.major_id FULL JOIN courses USING(course_id) GROUP BY course HAVING COUNT(student_id) = 1 ORDER BY course")"
